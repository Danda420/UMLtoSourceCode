...
namespace UMLtoSourceCode.Class
{
    ...
    {
        public static bool Point1(xtUMLtoCSharp form1, JArray jsonArray)
        {
                ...
                foreach (var item in jsonArray)
                {
                    if (item["type"]?.ToString() == "subsystem")
                    {
                        ...
                        if (subsystemProperty != null)
                        {
                            ...
                            {
                                msgBox.AppendLine($"Syntax error 1: There is a subsystem with the same name: {subsystemName}. Ensure that all subsystems have unique names.\r\n");
                            }
                        ...
                        {
                            msgBox.AppendLine("Syntax error 1: Property 'sub_name' not found or is empty.\r\n");
                        }
                    }
                }
            ...
            {
                msgBox.AppendLine("Syntax error 1: " + ex.Message + "\r\n");
                return false;
            }
        }
        public static bool Point2(xtUMLtoCSharp form1, JArray jsonArray)
        {
            ...
            try
            {
                foreach (var subsystem in jsonArray)
                {
                        ...
                        foreach (var item in modelArray)
                        {
                            ...
                            if (itemType == "class")
                            {
                                ...
                                if (classIdProperty != null)
                                {
                                    ...
                                    {
                                        msgBox.AppendLine($"Syntax error 2: Class {classId} in subsystem {subsystem["sub_name"]?.ToString()} does not have attributes.\r\n");
                    ...
                    {
                        msgBox.AppendLine($"Syntax error 2: There are classes without relationships in subsystem {subsystem["sub_name"]?.ToString()}. Class IDs without relationships: {string.Join(", ", classesWithoutRelation)}\r\n");
                    }
                }
            ...
            {
                msgBox.AppendLine("Syntax error 2: " + ex.Message + "\r\n");
                return false;
            }
        }

        public static bool Point3(xtUMLtoCSharp form1, JArray jsonArray)
        {
            ...
            try
            {
                ...
                processItem = (item) =>
                {
                    ...
                    if (itemType == "class")
                    {
                        ...
                        {
                            msgBox.AppendLine($"Syntax error 3: Class {className} in subsystem {item["sub_name"]?.ToString()} has the same information as a class in another subsystem.\r\n");
                    ...
                    else if (itemType == "association_class")
                    {
                        ...
                        if (classInfoMap.ContainsKey(classInfo))
                        {
                            msgBox.AppendLine($"Syntax error 3: Class {className} in subsystem {item["sub_name"]?.ToString()} has identical information to a class in another subsystem.\r\n");
            ...
            {
                msgBox.AppendLine("Syntax error: " + ex.Message + "\r\n");
                return false;
            }
        }
        ...
        public static bool Point4(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                foreach (var subsystem in jsonArray)
                {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if ((itemType == "class" || itemType == "association_class") && item["class_name"] != null)
                        {
                            ...
                            {
                                msgBox.AppendLine("Syntax error 4: Class name or class_id is empty in the subsystem. \r\n");

                            }

                            if (classNames.Contains(className))
                            {
                                msgBox.AppendLine($"Syntax error 4: Duplicate class name {className} within this subsystem. \r\n");

                            }

                            if (classIds.Contains(classId))
                            {
                                msgBox.AppendLine($"Syntax error 4: Duplicate class_id {classId} within this subsystem. \r\n");
                        ...
                        if (itemType == "association" && item["model"] is JObject associationModel)
                        {
                            ...
                            if (associationItemType == "association_class" && associationModel["class_name"] != null)
                            {
                                ...
                                {
                                    msgBox.AppendLine($"Syntax error 4: Duplicate class name {associationClassName} within this subsystem. \r\n");

                                }

                                if (classIds.Contains(associationClassId))
                                {
                                    msgBox.AppendLine($"Syntax error 4: Duplicate class_id {associationClassId} within this subsystem. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 4: " + ex.Message + "\r\n");
                return false;
            }
        }

        public static bool Point5(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                foreach (var subsystem in jsonArray)
                {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "class" && item["class_name"] != null)
                        {
                            ...
                            {
                                msgBox.AppendLine("Syntax error 5: Class name or KL is empty in the subsystem. \r\n");
                            }

                            if (uniqueKLs.Contains(KL))
                            {
                                msgBox.AppendLine($"Syntax error 5: Duplicate KL value {KL} within this subsystem. \r\n");
                            }
                        ...
                        if (itemType == "association" && item["model"] is JObject associationModel)
                        {
                            ...
                            if (associationItemType == "association_class" && associationModel["class_name"] != null)
                            {
                                ...
                                {
                                    msgBox.AppendLine("Syntax error 5: KL value is empty in the subsystem. \r\n");
                                }

                                if (uniqueKLs.Contains(associationKL))
                                {
                                    msgBox.AppendLine($"Syntax error 5: Duplicate KL value {associationKL} within this subsystem. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 5: " + ex.Message + "\r\n");
                return false;
            }
        }
