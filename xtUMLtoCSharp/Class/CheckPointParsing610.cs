...
namespace UMLtoSourceCode.Class
{
    ...
    {
        public static bool Point6(xtUMLtoCSharp form1, JArray jsonArray)
        {
                        ...
                        if (itemType == "class" && item["class_name"] != null)
                        {
                            ...
                            if (attributes != null)
                            {
                                ...
                                foreach (var attribute in attributes)
                                {
                                    ...
                                    {
                                        msgBox.AppendLine($"Syntax error 6: Attribute name is empty in class {className}. \r\n");

                                    }

                                    if (uniqueAttributeNames.Contains(attributeName))
                                    {
                                        msgBox.AppendLine($"Syntax error 6: Duplicate attribute name {attributeName} in class {className}. \r\n");
                            ...
                            if (associationItemType == "association_class" && associationModel["class_name"] != null)
                            {
                                ...
                                if (associationAttributes != null)
                                {
                                    ...
                                    foreach (var attribute in associationAttributes)
                                    {
                                        ...
                                        if (string.IsNullOrWhiteSpace(attributeName))
                                        {
                                            msgBox.AppendLine($"Syntax error 6: Attribute name is empty in association class {associationClassName}. \r\n");

                                        }

                                        if (uniqueAssociationAttributeNames.Contains(attributeName))
                                        {
                                            msgBox.AppendLine($"Syntax error 6: Duplicate attribute name {attributeName} in association class {associationClassName}. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 6: " + ex.Message + "\r\n");
                return false;
            }
        }

        public static bool Point7(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "class" && item["class_name"] != null)
                        {
                            ...
                            if (attributes != null)
                            {
                                ...
                                if (!hasPrimaryKey)
                                {
                                    msgBox.AppendLine($"Syntax error 7: Class {className} does not have a primary key. \r\n");

                                }
                            }
                            else
                            {
                                msgBox.AppendLine($"Syntax error 7: Class {className} does not have any attributes. \r\n");
                        ...
                        if (itemType == "association" && item["model"] is JObject associationModel)
                        {
                            ...
                            if (associationItemType == "association_class" && associationModel["class_name"] != null)
                            {
                                ...
                                if (associationAttributes != null)
                                {
                                    ...
                                    if (!hasPrimaryKey)
                                    {
                                        msgBox.AppendLine($"Syntax error 7: Association Class {associationClassName} does not have a primary key. \r\n");

                                    }
                                }
                                else
                                {
                                    msgBox.AppendLine($"Syntax error 7: Association Class {associationClassName} does not have any attributes. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 7: " + ex.Message + "\r\n");
                return false;
            }
        }

        public static bool Point8(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                foreach (var subsystem in jsonArray)
                {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "association" && item["name"] != null)
                        {
                            ...
                            {
                                msgBox.AppendLine("Syntax error 8: Association name is empty in the subsystem. \r\n");

                            }

                            if (associationNames.Contains(associationName))
                            {
                                msgBox.AppendLine($"Syntax error 8: Duplicate association name {associationName} within this subsystem.\r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 8: " + ex.Message + "\r\n");
                return false;
            }
        }

        public static bool Point9(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "association")
                        {
                            ...
                            if ((class1Multiplicity == "0..*" && class2Multiplicity == "0..*") || (class1Multiplicity == "0..*" && class2Multiplicity == "1..*") || (class1Multiplicity == "1..*" && class2Multiplicity == "0..*") || (class1Multiplicity == "1..*" && class2Multiplicity == "1..*"))
                            {
                                ...
                                {
                                    msgBox.AppendLine($"Syntax error 9: Relationship {item["name"]?.ToString()} (many-to-many) has not been formalized with an association_class. \r\n");
                                }
                                     ...
                                     (class1Multiplicity == "1..1" && class2Multiplicity == "1..1"))
                            {
                                ...
                                {
                                    msgBox.AppendLine($"Syntax error 9: One of the Class {class1Id} or {class2Id} in relationship {item["name"]?.ToString()} (one-to-one) must be formalized with a referential_attribute. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 9: " + ex.Message + "\r\n");
                return false;
            }
        }
        ...
        public static bool Point10(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            ...
            try
            {
                ...
                {
                    msgBox.AppendLine("Syntax error 10: JSON array is null or empty.\r\n");
                    return false;
                }
                ...
                foreach (string attributeName in referentialAttributeNames)
                {
                    ...
                    {
                        msgBox.AppendLine($"Syntax error 10: Referential attribute '{attributeName}' has incorrect naming.\r\n");
                        return false;
                    }
            ...
            {
                msgBox.AppendLine("Syntax error 10: " + ex.Message + "\r\n");
                return false;
            }
        }
