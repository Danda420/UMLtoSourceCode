...
namespace UMLtoSourceCode.Class
{
    ...
    {
        public static bool Point11(xtUMLtoCSharp form1, JArray subsystems)
        {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "association")
                        {
                            ...
                            if (!IsClassInSubsystem(subsystem, class1Id))
                            {
                                ...
                                {
                                    msgBox.AppendLine($"Syntax error 11: Subsystem {subsystemId} does not have a corresponding class or imported class for relationship {item["name"]?.ToString()}. \r\n");
                            ...
                            if (!IsClassInSubsystem(subsystem, class2Id))
                            {
                                ...
                                {
                                    msgBox.AppendLine($"Syntax error 11: Subsystem {subsystemId} does not have a class or imported class corresponding to the relationship {item["name"]?.ToString()}. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 11: " + ex.Message + "\r\n");
                return false;
            }
        }
        ...
        public static bool Point13(xtUMLtoCSharp form1, JArray jsonArray)
        {
            StringBuilder msgBox = form1.GetMessageBox();
                    ...
                    if (subsistem?["type"]?.ToString() == "subsystem")
                    {
                        ...
                        foreach (var item in model)
                        {
                            if (item?["type"]?.ToString() == "class")
                            {
                                if (!CekKelas(item))
                                {
                                    msgBox.AppendLine($"Syntax error 13: Class or class attribute {item["class_name"]} is incomplete. \r\n");
                                }
                            }
                            else if (item?["type"]?.ToString() == "association")
                            {
                                if (!CekRelasi(item))
                                {
                                    msgBox.AppendLine($"Syntax error 13: Association class or relationship {item["name"]} is incomplete. \r\n");
                                }
                            }
                        }
                    }
            ...
            {
                msgBox.AppendLine("Syntax error 13: " + ex.Message + "\r\n");
                return false;
            }
        }
        ...
        public static bool Point14(xtUMLtoCSharp form1, JArray subsystems)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                    ...
                    foreach (var item in currentSubsystem["model"])
                    {
                        ...
                        if (itemType == "association")
                        {
                            ...
                            if (!IsClassInSubsystem(currentSubsystem, class1Id))
                            {
                                ...
                                {
                                    msgBox.AppendLine($"Syntax error 14: Subsystem {currentSubsystem["sub_name"]} does not have a corresponding class or imported class for the relationship {item["name"]?.ToString()}.\r\n");

                                }

                                if (!IsRelationshipInOtherSubsystem(subsystems, currentSubsystem, class1Id, class2Id))
                                {
                                    msgBox.AppendLine($"Syntax error 14: Subsystem {currentSubId} has a relationship with class_id {class1Id} or {class2Id}, but there is no corresponding relationship in other subsystems. \r\n");
            ...
            {
                msgBox.AppendLine("Syntax error 14: " + ex.Message + "\r\n");
                return false;
            }
        }
        ...
        public static bool Point15(xtUMLtoCSharp form1, JArray subsystems)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                foreach (var subsystem in subsystems)
                {
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "class" && item["states"] is JArray states)
                        {
                            ...
                            msgBox.AppendLine($"Syntax error 15: Subsystem {subsystem["sub_id"]?.ToString()} has a class {className} without a corresponding state. \r\n");
            ...
            {
                msgBox.AppendLine($"Syntax error 15: {ex.Message} \r\n");
                return false;
            }
        }

        public static bool Point99(xtUMLtoCSharp form1, JArray subsystems)
        {
            StringBuilder msgBox = form1.GetMessageBox();
            try
            {
                foreach (var subsystem in subsystems)
                {
                    ...
                    foreach (var item in subsystem["model"])
                    {
                        ...
                        if (itemType == "association")
                        {
                            ...
                            {
                                msgBox.AppendLine($"Syntax error 99: Subsystem {subsystemId} has an association {item["name"]?.ToString()} that lacks a relationship between two classes within it. \r\n");
            ...
            {
                msgBox.AppendLine($"Syntax error 99: {ex.Message} \r\n");
                return false;
            }
        }
